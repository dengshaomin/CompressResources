package com.code.cpimage.webp

import com.code.cpimage.Config
import com.code.cpimage.Const
            ...
            if (ImageUtil.isImage(imgFile)) {
                ...
                if (webpFile.length() < imgFile.length()) {
                    LogUtil.log(
                        TAG,
                        imgFile.path,
                        imgFile.length().toString(),
                        webpFile.length().toString()
                    )
                    if (imgFile.exists()) {
                        imgFile.delete()
                    }
        ...
        fun securityFormatWebp(imgFile: File, config: Config, project: Project) {
            ...
            if (ImageUtil.isImage(imgFile)) {
                if (config.isSupportAlphaWebp) {
                    ...
                    if (imgFile.name.endsWith(Const.JPG) || imgFile.name.endsWith(
                            Const.JPEG)) {
                        //jpg
                        formatWebp(
                            imgFile
